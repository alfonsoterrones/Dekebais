 ...
 * @author Beto Rojas
 */
public class ExcelMorososQuintoPago {

	public ExcelMorososQuintoPago() {
		...
		try {
			...
			ResultSet rset = stm.executeQuery(
					"select NOMBREEMPLE, APELLIDOPRIMERO, APELLIDOSEGUNDO, SALDO, TELEFONO,  NUEVO, EDAD from COMPONENTE");
			...
			int contadores = 1;
			while (rset.next()) {
				if (rset.getInt("SALDO") < 100 && rset.getInt("EDAD") > 11 || (rset.getInt("EDAD") < 12
		...
		try {
			...
			FileOutputStream archivo = new FileOutputStream("MosososQuintoPago.xls");
			libro.write(archivo);
			archivo.close();
