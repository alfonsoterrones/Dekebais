 ...
 * @author Beto Rojas
 */
public class ExcelMorososPrimerPago {

	public ExcelMorososPrimerPago() {
		...
		try {
			...
			int contadores = 1;
			while (rset.next()) {
				if (rset.getInt("SALDO") < 20 && rset.getInt("EDAD") > 11 || (rset.getInt("EDAD") < 12
		...
		try {
			...
			FileOutputStream archivo = new FileOutputStream("MosososPrimerPago.xls");
			libro.write(archivo);
			archivo.close();
