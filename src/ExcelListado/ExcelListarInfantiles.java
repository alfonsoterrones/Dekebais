 ...
 * @author Beto Rojas
 */
public class ExcelListarInfantiles {

	public ExcelListarInfantiles() {
		...
		// Creamos una instancia de la clase HSSFRow llamada fila y creamos la fila con
		// el indice 0
		...
		try {
			...
			ResultSet rset = stm.executeQuery(
					"select NOMBREEMPLE, APELLIDOPRIMERO, APELLIDOSEGUNDO, MAIL, FECHNACIMIENTO, SALDO, AMIGO, TELEFONO, CUERPO, NUEVO, EDAD ,MODISTA,COMPONENTE from COMPONENTE");
			...
			while (rset.next()) {
				...
				if ((ano > anoCarnaval || (ano == anoCarnaval && mes > mesCarnaval)
						|| (ano == anoCarnaval && mes == mesCarnaval && dia > diaCarnaval))
						&& rset.getString("COMPONENTE").equals("Si")) {
		...
		try {
			...
			String ex = "Componentes2017";

			do {
				// ex = JOptionPane.showInputDialog(null, "Nombre del
				// Excel",JOptionPane.QUESTION_MESSAGE);
				if (ex == null) {
					JOptionPane.showMessageDialog(null, "Te falto introducir el nombre del Excel", "Error", 2);
				}
			} while (ex == null);
			FileOutputStream archivo = new FileOutputStream("Infantiles2017.xls");
			libro.write(archivo);
			archivo.close();
