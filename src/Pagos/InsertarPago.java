...
public class InsertarPago{
	 ...
	 public InsertarPago(String nombre,String apellido,Float pago,String quePago){
		...
		float saldoFinal=0f;
		 ...
		 ResultSet rset=stm.executeQuery("select NOMBREEMPLE, APELLIDOPRIMERO, APELLIDOSEGUNDO, MAIL, FECHNACIMIENTO, SALDO, AMIGO,NUEVO from DAW1.COMPONENTE");
		 ...
		 while (rset.next()){
			 ...
			 if((rset.getString("NOMBREEMPLE").equals(nombre) || rset.getString("NOMBREEMPLE").equals(nombre+" ")) && (rset.getString("APELLIDOPRIMERO").equals(apellido+" ") || rset.getString("APELLIDOPRIMERO").equals(apellido))){
	                ...
	                //JOptionPane.showMessageDialog(null, "Nombre"+nombre+"nombre"+rset.getString("NOMBREEMPLE")+"el if Apellido"+apellido+"apellido"+rset.getString("APELLIDOPRIMERO")+"el if", "Aviso",1);
	                String saldoFinalString = String.valueOf(saldoFinal); 
	       		 	String consulta="update componente set saldo="+saldoFinalString+" where nombreemple='"+nombre+"' and APELLIDOPRIMERO='"+apellido+"'";
