					 ...
					 while (rset.next()){
							if( (rset.getString ("NOMBREEMPLE")+" "+rset.getString ("APELLIDOPRIMERO")).equals(componente)){
								if(rdbtnSi.getSelectedObjects()!=null){
									consulta="UPDATE COMPONENTE SET COMPONENTE='Si' WHERE NOMBREEMPLE='"+rset.getString("NOMBREEMPLE")+"' AND APELLIDOPRIMERO='"+rset.getString("APELLIDOPRIMERO")+"' AND APELLIDOSEGUNDO='"+rset.getString("APELLIDOSEGUNDO")+"'";
					                stm.executeUpdate(consulta);
					                setVisible(false);
					                JOptionPane.showMessageDialog(null, "Otro más que sale este año en Cambalada\n uii en Dekebais!!", "Aviso",1);
								}else{
									if(rdbtnNo.getSelectedObjects()!=null){
										consulta="UPDATE COMPONENTE SET COMPONENTE='No' WHERE NOMBREEMPLE='"+rset.getString("NOMBREEMPLE")+"' AND APELLIDOPRIMERO='"+rset.getString("APELLIDOPRIMERO")+"' AND APELLIDOSEGUNDO='"+rset.getString("APELLIDOSEGUNDO")+"'";
						                stm.executeUpdate(consulta);
						                setVisible(false);
						                JOptionPane.showMessageDialog(null, "Ala ya te lo has cargado\n a toma por culo este año!!", "Aviso",1);
