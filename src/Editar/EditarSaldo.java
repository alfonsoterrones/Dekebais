...
public class EditarSaldo extends JFrame {
	...
	public EditarSaldo(String componente) {
					...
					boolean semaforo=true;
					 while (rset.next()){
							if( (rset.getString ("NOMBREEMPLE")+" "+rset.getString ("APELLIDOPRIMERO")).equals(componente)){
									consulta="UPDATE COMPONENTE SET SALDO="+textField.getText()+" WHERE NOMBREEMPLE='"+rset.getString("NOMBREEMPLE")+"' AND APELLIDOPRIMERO='"+rset.getString("APELLIDOPRIMERO")+"' AND APELLIDOSEGUNDO='"+rset.getString("APELLIDOSEGUNDO")+"'";
					                stm.executeUpdate(consulta);
					                setVisible(false);
					                JOptionPane.showMessageDialog(null, "Le has añadido el dinero a Alfonso Terrones!!", "Aviso",1);
		...
		lblEliminaTodosLos = new JLabel("Elimina todos los pagos de "+componente+" que haya pagado para que tenga que pagar todo de nuevo xD");
		GroupLayout gl_contentPane = new GroupLayout(contentPane);
		gl_contentPane.setHorizontalGroup(
			gl_contentPane.createParallelGroup(Alignment.LEADING)
				.addGroup(Alignment.TRAILING, gl_contentPane.createSequentialGroup()
					.addContainerGap(200, Short.MAX_VALUE)
					.addComponent(lblEliminaTodosLos)
					.addGap(166))
				.addGroup(gl_contentPane.createSequentialGroup()
					.addContainerGap()
					.addComponent(lblCuantoDineroLleva)
					.addGap(111)
