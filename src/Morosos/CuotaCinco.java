...
import ExcelListado.ExcelMorososQuintoPago;
...
public class CuotaCinco extends JFrame{
	...
	public CuotaCinco(){
		super("Componentes que faltan por pagar el quinto pago");
		setSize(400, 400);
		setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
		setVisible(true);
		setLocation(600,400);
		contentPane = new Fondo("Morosos4.jpg");
		...
		cursos.addMouseListener(new MouseAdapter() {
		 ...
		 try {
			 ...
			 ResultSet rset=stm.executeQuery("select NOMBREEMPLE, APELLIDOPRIMERO, SALDO, NUEVO, EDAD from COMPONENTE order by NOMBREEMPLE");
			 
			 cursos.removeAllItems();
			 while (rset.next()){
				 if(rset.getInt("SALDO")<100 && rset.getInt("EDAD")>11 || (rset.getInt("EDAD")<12 && rset.getString("NUEVO").equals("No") && rset.getInt("SALDO")<50)){
			 ...
			 btnImprimir.addMouseListener(new MouseAdapter() {
			 	@Override
			 	public void mousePressed(MouseEvent e) {
			 		ExcelMorososQuintoPago quin = new ExcelMorososQuintoPago();
			 	}
			 });
