...
import ExcelListado.ExcelMorososCuartoPago;
...
public class CuotaCuatro extends JFrame {
	String nombreapellido;
	String componente;
	private Fondo contentPane;

	public CuotaCuatro() {
		super("Componentes que faltan por pagar el cuarto pago");
		setSize(466, 458);
		setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
		setVisible(true);
		setLocation(600, 400);
		contentPane = new Fondo("morosos7.png");
		...
		try {
			...
			while (rset.next()) {
				if (rset.getInt("SALDO") < 80 && rset.getInt("EDAD") > 11 || (rset.getInt("EDAD") < 12
			...
			btnImprimir.addMouseListener(new MouseAdapter() {
				@Override
				public void mousePressed(MouseEvent e) {
					ExcelMorososCuartoPago cuar = new ExcelMorososCuartoPago();
				}
			});
			...
			gl_contentPane.setHorizontalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING)
					...
					.addGroup(Alignment.TRAILING, gl_contentPane.createSequentialGroup()
							.addContainerGap(329, Short.MAX_VALUE).addComponent(btnImprimir).addContainerGap()));
