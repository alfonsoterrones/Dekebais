	...
	public ListarMorosos() {
		...
		try {
			...
			ResultSet rset = stm.executeQuery(
					"select NOMBREEMPLE, APELLIDOPRIMERO, SALDO, NUEVO from COMPONENTE order by NOMBREEMPLE");

			cursos.removeAllItems();
			while (rset.next()) {
				if ((rset.getInt("SALDO") < 100 && rset.getString("NUEVO").equals("No"))
						|| (rset.getInt("SALDO") < 106 && rset.getString("NUEVO").equals("Si"))) {
